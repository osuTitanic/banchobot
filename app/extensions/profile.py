...
class Profile(BaseCog):
    ...
    async def profile(self, ctx: commands.Context, username: str | None = None) -> None:
        if not (user := await self.resolve_user_with_stats(username)):
            message = (
    ...
    async def resolve_user_with_stats(self, username: str | None = None) -> DBUser | None:
        with self.database.managed_session() as session:
            user = (
                await self.resolve_user(self.bot.user.id, session) if username is None else
                await self.resolve_user_by_name(username, session)
            )
