...
async def pp_record(context: Context):
    ...
    with app.session.database.managed_session() as session:
        if context.args:
            ...
            records = [
                scores.fetch_pp_record(
                    mode,
                    Mods.from_string(context.args[0]),
                    session=session
                )
                for mode in range(4)
            ]
        ...
        standard = (
            top_score(mode=0, mods=0, exclude=[Mods.Relax, Mods.Autopilot], session=session),
            top_score(mode=0, mods=Mods.Relax, session=session),
            top_score(mode=0, mods=Mods.Autopilot, session=session),
        )

        taiko = (
            top_score(mode=1, mods=0, exclude=[Mods.Relax], session=session),
            top_score(mode=1, mods=Mods.Relax, session=session),
        )

        ctb = (
            top_score(mode=2, mods=0, exclude=[Mods.Relax], session=session),
            top_score(mode=2, mods=Mods.Relax, session=session),
        )
